...
const AnimatedFlatList = ScrollView;
...
export class Carousel extends Component {
    ...
    _getWrappedRef () {
        ...
        return this._carouselRef && this._carouselRef.current;
    }
    ...
    _initPositionsAndInterpolators (props = this.props) {
        ...
        this._getCustomData(props).forEach((itemData, index) => {
            ...
            if (!this._shouldAnimateSlides(props)) {
                animatedValue = Animated.createValue(1);
            } else if (this._shouldUseCustomAnimation()) {
                animatedValue = Animated.createValue(_index === this._activeItem ? 1 : 0);
            } else {
                let interpolator;
                ...
                animatedValue = Animated.interpolate(this._scrollPos, interpolator.inputRange, interpolator.outputRange);
            }
    ...
    _getSlideAnimation (index, toValue) {
        ...
        return Animated.parallel([
            Animated['timing'](
                animatedValue,
                { ...animationCommonOptions, easing: Animated.Easing.linear }
            ),
    ...
    _scrollTo (offset, animated = true) {
        ...
        if (vertical) {
            wrappedRef.setScrollTop(offset, animated);
        } else {
            wrappedRef.setScrollLeft(offset, animated);
        }
    }

    _onScroll (newScrollTop, newScrollLeft) {
        // scrollEnd timeout
        if (this._scrollTimeout) {
          clearTimeout(this._scrollTimeout);
        }
        this._scrollTimeout = setTimeout(this._onScrollEnd, 50);

        const { callbackOffsetMargin, enableMomentum, onScroll, vertical } = this.props;

        const scrollOffset = vertical ? newScrollTop : newScrollLeft;
    ...
    _getComponentStaticProps () {
        ...
        const containerStyle = [
            ...
            vertical ?
                ...
                { display: 'flex', width: sliderWidth, flexDirection: this._needsRTLAdaptations() ? 'row-reverse' : 'row' }
        ];
    ...
    render () {
        const { data, renderItem, vertical } = this.props;
